...
Formally we define the regret $y_{T,i}$ for the agent $\mathcal A$ with respect to expert $E_i\in\mathcal E$ (assumed finite for the moment) as follows:
...
\begin{equation}
	y_T=\max\limits_{E_i\in\mathcal E}y_{T,i}=L_T-\min\limits_{E_i\in\mathcal E}L_{T,i}
\end{equation}
...
A first simple counterexample can be found in~\cite{cover1966behavior}. If the decision space $\mathcal D$ is finite then there exists a sequence of loss function such that $y_T=\Omega(T)$.
% Cover Impossibility
Again take $\mathcal D$ as a space of binary outcomes, absolute loss as $f(x,y)=|x - y|$, and the class of experts is composed by two experts, one predicting always $0$ and the other always $1$. Taking $T$ odd, we have that the loss of the best expert is $L_{T,i}<\frac{T}{2}$, and we have already shown that the adversary can make the loss of the learner $L_T=T$. It is now evident that the regret is $y_T>T-\frac{T}{2}$, which do not allow $y_T/T\to 0$. This argument is easily extended in the case of any finite decision space $\mathcal D$.
...
\begin{equation}
    y_T = \sup\limits_{q\in\Delta_{N-1}}y_{T,q}:=L_T-\inf\limits_{q\in\Delta_{N-1}}L_{T,q},
\end{equation}
...
\begin{proof}
    ...
    \end{equation}
    Applying $\varphi(x)=\frac{1}{\nu}log(x)$ to both sides of equation \eqref{eq:conc_exp_last} we obtain that $$\Phi(\mathbf y_{t})\le \Phi(\mathbf y_{t-1})$$ that prove the thesis.
\end{proof}
...
\begin{equation}
    y_T\le \frac{1}{\eta}\log\left(\sum\limits_{i=1}^N e^{\nu y_T,j}\right)=\Phi(\mathbf y_T)\le\Phi(\mathbf y_0) = \frac{log N}{\eta}
\end{equation}
...
\label{sec:GT}\todo{Forse da togliere}
