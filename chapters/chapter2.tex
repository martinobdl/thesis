...
\todo{definire $h_t$ come la storia}
...
% Also note that since the adversary has knowledge of your actions before the turn, and the decision space $\mathcal D$ is finite then the agent $\mathcal A$ has to randomize its prediction and hence he will present a probability distribution over the decision space $\mathcal D$ at each turn, and so we could redefine $\mathcal D$ as the probability simplex over the original finite decision space. In order to avoid this precision, let's define $\mathcal D$ as a general convex space embedded in some Euclidean space.
% The simplest example that fits under this framework is coin tosses prediction. Each expert $\{E_i\}_{i\in[N]}$ place a prediction $x_{t,E_i}\in\mathcal D=\{0,1\}$ on the outcome of the binary event $r_t\in\mathcal X=\{0,1\}$, then the agent $\mathcal A$ plays its own prediction $x_t$, and the adversary choose the outcome $r_t$ of the coin toss. The loss function could be choose as simple as $f(x,r_t)=\mathbb I\{r_t\neq x\}$. The aim of the learner is to bound the number of wrong guesses with respect to the best expert $E_i$.
% \todo{Migliorare esempio}
...
\begin{definition}(Weighted Avergae Forcester).\label{def:weighted_avg}
...
It can be shown \todo{e allora show} that the algorithm defined by update rule \eqref{eq:ewf}, and for a convex loss function $l(\cdot,y_y)$, gives the following guarantee on the regret:
...
\label{sec:GT}
In this section we explore the connection of the framework of Section \ref{sec:OnlineLearning} into a more classical repeated game framework. For the moment we thought at the adversary as a black box without any specific model in mind. The reason of this chapter is to clarify its role as a player in the game and to show the game theoretical properties of no-regret agents \todo{no rgetet che?}. Since in Online Learning the convention is to speak about losses, we shall speak about losses (minimization) also in the classical definitions of game theory instead of payoffs (maximization).
...
Due to the impossibility result of Cover \todo{non l'ho mai chiamato così} we have to work with the mixed extension formulation of the game. So from now on we take this step implicitly.
The taxonomy of game definition is quite extended and complex, we will focus on non-cooperative games \todo{cita qualcuno}. More specifically we will need the model for \emph{Zero Sum Game}. 
